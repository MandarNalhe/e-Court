...
const register = ()=>{
  const navigate = useNavigate();
    ...
    const handleOTP = async () => {
      try {
        const response = await fetch("https://backend-production-969c.up.railway.app/api/otp/send-otp", {
          method: "POST",
    ...
    const [loginData, setLoginData] = useState({
      ...
      const handleLoginSubmit = async (e) => {
        e.preventDefault();
        try {
          const response = await fetch("https://backend-production-969c.up.railway.app/api/auth/login", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(loginData),
          });
      ...
      const handleSubmit = async (e) => {
        ...
        try {
          const response = await fetch("https://backend-production-969c.up.railway.app/api/auth/register", {
            method: "POST",
            headers: {
